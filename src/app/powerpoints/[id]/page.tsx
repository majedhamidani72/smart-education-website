...
export default function PowerpointDetailsPage() {
  ...
  return <main className="min-h-screen bg-gradient-to-b from-slate-50 to-white px-4 py-8"><div className="mx-auto max-w-7xl">
    ...
    <section className="mt-6 grid gap-7 rounded-[2rem] border border-slate-100 bg-white p-5 shadow-xl shadow-slate-100/70 lg:grid-cols-[1.08fr_0.92fr] lg:p-8">
      ...
      <div className="flex flex-col"><div className="flex flex-wrap gap-2 text-xs font-bold"><span className="rounded-full bg-indigo-50 px-3 py-2 text-indigo-700">پایه {item.grade.title}</span><span className="rounded-full bg-orange-50 px-3 py-2 text-orange-700">{item.book.title}</span><span className="rounded-full bg-emerald-50 px-3 py-2 text-emerald-700">{item.chapter.title}</span></div><h1 className="mt-5 text-xl font-black leading-10 text-slate-950 sm:text-2xl">{item.title}</h1><p className="mt-4 text-sm leading-8 text-slate-600">{item.description || 'پاورپوینت آماده و منظم برای تدریس این فصل در کلاس.'}</p>
        ...
        <div className="mt-auto pt-7"><div className="flex items-end gap-3">{item.discount_percent > 0 && <del className="text-sm text-slate-400">{item.price.toLocaleString('fa-IR')} تومان</del>}<strong className="text-xl font-black text-indigo-700">{item.final_price.toLocaleString('fa-IR')} تومان</strong></div>{item.owned ? <button onClick={() => downloadPowerpoint(item).catch((error) => setMessage(error.message))} className="mt-5 flex w-full items-center justify-center gap-2 rounded-2xl bg-emerald-600 py-4 font-black text-white"><Download size={20} /> دانلود پاورپوینت خریداری‌شده</button> : <button onClick={() => { setAccepted(false); setLicenseOpen(true); }} className="mt-5 flex w-full items-center justify-center gap-2 rounded-2xl bg-indigo-700 py-4 font-black text-white shadow-lg shadow-indigo-100"><ShoppingBag size={20} /> خرید این پاورپوینت</button>}</div>
      </div>
    </section>
    <section className="mt-8 rounded-[2rem] border border-indigo-100 bg-white p-5 shadow-sm sm:p-8"><div className="flex flex-wrap items-center justify-between gap-4"><div><span className="text-xs font-black text-indigo-600">قبل از خرید ببینید</span><h2 className="mt-1 text-lg font-black text-slate-900">نمونه واقعی پاورپوینت</h2><p className="mt-2 text-sm text-slate-500">نمونه به‌صورت PDF چنداسلایدی نمایش داده می‌شود، نه تصاویر پراکنده.</p></div>{item.has_preview && <button onClick={() => setPreviewFullscreen(true)} className="inline-flex items-center gap-2 rounded-xl border border-indigo-100 px-4 py-3 text-sm font-black text-indigo-700"><Expand size={18} /> نمایش بزرگ</button>}</div>{item.has_preview ? <div className="mt-6 overflow-hidden rounded-2xl border border-slate-200 bg-slate-100"><iframe src={previewUrl} title={`نمونه ${item.title}`} className="h-[70vh] min-h-[520px] w-full" /></div> : <div className="mt-6 flex min-h-64 flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50 text-center"><LockKeyhole size={38} className="text-slate-300" /><strong className="mt-4 text-slate-700">هنوز PDF نمونه ثبت نشده است</strong><span className="mt-2 text-sm text-slate-400">پس از بارگذاری نمونه در پنل، همین‌جا قابل ورق‌زدن خواهد بود.</span></div>}</section>
  </div>
