...
export default function DashboardPage() {
  ...
  if (error) return <main className="mx-auto max-w-xl px-4 py-12 text-center"><div className="rounded-3xl border border-rose-100 bg-rose-50 p-7 text-sm font-bold text-rose-700">{error}</div></main>;
  if (!data) return <main className="min-h-[65vh] px-4 py-14 text-center text-sm text-slate-500">در حال آماده‌کردن مسیر یادگیری تو...</main>;
  ...
  return <main className="min-h-screen bg-gradient-to-b from-sky-50 via-white to-orange-50/40 px-4 py-6 sm:py-11"><div className="mx-auto max-w-6xl">
    <header className="flex flex-col gap-4 rounded-[2rem] bg-gradient-to-l from-indigo-700 to-violet-700 p-6 text-white shadow-xl shadow-indigo-100 sm:flex-row sm:items-center sm:justify-between sm:p-8"><div><span className="text-xs font-bold text-indigo-200">داشبورد یادگیری من</span><h1 className="mt-1 text-lg font-black">سلام {data.student.name} 👋</h1><p className="mt-2 text-sm text-indigo-100">یک قدم کوچک دیگر تا یادگیری بهتر</p></div><Link href={resumeHref} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-white px-5 py-3 font-black text-indigo-700"><PlayCircle size={21} />{resume ? 'ادامه آخرین درس' : 'شروع یادگیری'}</Link></header>
...
function Empty({ text }: { text: string }) { return <p className="mt-4 rounded-2xl bg-slate-50 p-5 text-center text-xs leading-6 text-slate-500">{text}</p>; }
