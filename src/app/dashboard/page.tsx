...
export default function DashboardPage() {
  ...
  return <main className="min-h-screen bg-gradient-to-b from-sky-50 via-white to-orange-50/40 px-4 py-8 sm:py-11"><div className="mx-auto max-w-6xl">
    <header className="flex flex-col gap-4 rounded-[2rem] bg-gradient-to-l from-indigo-700 to-violet-700 p-6 text-white shadow-xl shadow-indigo-100 sm:flex-row sm:items-center sm:justify-between sm:p-8"><div><span className="text-xs font-bold text-indigo-200">داشبورد یادگیری من</span><h1 className="mt-1 text-xl font-black">سلام {data.student.name} 👋</h1><p className="mt-2 text-sm text-indigo-100">یک قدم کوچک دیگر تا یادگیری بهتر</p></div><Link href={resumeHref} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-white px-5 py-3 font-black text-indigo-700"><PlayCircle size={21} />{resume ? 'ادامه آخرین درس' : 'شروع یادگیری'}</Link></header>
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-[1.4fr_.8fr]">
      ...
      <section className="rounded-3xl border border-slate-100 bg-white p-5 shadow-sm sm:p-6"><h2 className="flex items-center gap-2 text-base font-black"><BarChart3 className="text-sky-600" />روند آزمون‌ها</h2>{data.chart.length ? <div className="mt-6 flex h-44 items-end gap-2">{data.chart.map((point) => <div key={point.label} className="flex h-full flex-1 flex-col justify-end gap-2 text-center"><span className="text-[10px] font-bold text-slate-500">{point.percentage}٪</span><div className="min-h-1 rounded-t-lg bg-gradient-to-t from-indigo-600 to-sky-400" style={{ height: `${Math.max(5, point.percentage)}%` }} /><span className="truncate text-[9px] text-slate-400">{point.label}</span></div>)}</div> : <Empty text="پس از اولین آزمون، روند پیشرفت اینجا نمایش داده می‌شود." />}</section>
    </div>
