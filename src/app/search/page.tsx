...
export default function SearchPage() {
  ...
  return <main className="min-h-screen bg-slate-50 px-4 py-6 sm:py-11"><div className="mx-auto max-w-6xl">
    <header className="rounded-[2rem] bg-gradient-to-l from-sky-700 to-indigo-700 p-6 text-white sm:p-8"><span className="text-xs font-bold text-sky-200">دسترسی سریع درسکا</span><h1 className="mt-1 text-lg font-black sm:text-xl">هر چیزی را سریع پیدا کن</h1><p className="mt-2 text-sm text-sky-100">نام درس، عنوان محتوا یا شماره صفحه را بنویس.</p><label className="mt-4 flex items-center gap-3 rounded-2xl bg-white px-4 py-3 text-slate-800 shadow-lg"><Search className="text-indigo-600" /><input value={query} onChange={(event) => setQuery(event.target.value)} autoFocus placeholder="مثلاً عددنویسی، ستایش یا صفحه ۱۲" className="w-full bg-transparent text-sm font-bold outline-none placeholder:font-normal placeholder:text-slate-400" />{query && <button onClick={() => setQuery('')} aria-label="پاک‌کردن جست‌وجو"><X size={18} className="text-slate-400" /></button>}</label></header>
