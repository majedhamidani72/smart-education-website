...
export default function QuizPage() {
  ...
  if (phase.name === 'needs-purchase') {
    return (
      <div className="mx-auto max-w-lg px-4 py-16 text-center">
        ...
        {bookId && <Link href={`/purchase?book_id=${bookId}&return_to=${encodeURIComponent(`/quiz/${params.id}?book_id=${bookId}`)}`} className="mt-6 inline-flex rounded-xl bg-violet-700 px-6 py-3 text-sm font-bold text-white">مشاهده پلن‌های خرید</Link>}
      </div>
    );
  }
  ...
  return (
    <main className="min-h-[calc(100vh-74px)] bg-gradient-to-b from-violet-50 via-white to-sky-50 px-4 py-7 sm:py-10">
...
function ResultView({ result, bookId }: { result: QuizResult; bookId: number | null }) {
  ...
  return (
    <main className="min-h-[calc(100vh-74px)] bg-slate-50 px-4 py-7 text-slate-800 sm:py-10">
      <div className="mx-auto max-w-4xl">
        <section className="rounded-3xl border border-emerald-100 bg-white p-6 shadow-sm sm:p-8">
          <div className="flex flex-col items-center gap-4 text-center sm:flex-row sm:text-right">
            <span className="flex h-20 w-20 shrink-0 items-center justify-center rounded-full bg-emerald-50 text-5xl">{face}</span>
            <div className="flex-1"><p className="text-xs font-bold text-emerald-600">نتیجه آزمون</p><h1 className="mt-1 text-2xl font-black text-slate-900">{assessment.label}</h1><p className="mt-2 text-sm leading-7 text-slate-500">{assessment.message}</p></div>
            <div className="border-t border-slate-100 pt-4 text-center sm:border-r sm:border-t-0 sm:pr-7 sm:pt-0"><strong className="block text-4xl font-black text-teal-700">{percentage.toLocaleString('fa-IR')}٪</strong><span className="mt-1 block text-xs text-slate-400">{result.score.earned.toLocaleString('fa-IR')} از {result.score.total.toLocaleString('fa-IR')} امتیاز</span></div>
          </div>
        </section>
