  ...
  return (
    <div className="overflow-hidden bg-white">
      ...
      {learningOptions.length > 0 ? (
        ...
        <section id="learning-explorer" className="grade-pattern px-4 py-10"><div className="mx-auto max-w-2xl rounded-2xl border border-red-100 bg-white p-5 text-center text-sm text-red-700 shadow-sm">{backendOffline ? 'اتصال به سرور برقرار نشد. لطفاً بک‌اند Laravel را اجرا کنید.' : 'هنوز پایه‌ای برای نمایش ثبت نشده است.'}</div></section>
      )}

      <section id="powerpoint-showcase" className="relative overflow-hidden bg-gradient-to-b from-[#fff9f2] to-white px-4 py-10 sm:py-14">
        ...
        <div className="relative mx-auto max-w-[1480px]">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
            <div><span className="inline-flex items-center gap-2 rounded-full bg-orange-100 px-4 py-2 text-xs font-black text-orange-700"><Presentation size={17} /> ویژه معلمان</span><h2 className="mt-4 text-lg font-black text-slate-900 sm:text-2xl">پاورپوینت‌های آماده تدریس</h2><p className="mt-3 max-w-2xl text-sm leading-7 text-slate-600">اسلایدهای مرتب و آماده هر فصل را ببینید و هر پاورپوینت را جداگانه تهیه کنید.</p></div>
          ...
          {powerpoints.length > 0 ? <div className="mt-7 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{powerpoints.map((item) => <Link href="/powerpoints" key={item.id} className="group overflow-hidden rounded-[1.7rem] border border-orange-100 bg-white shadow-[0_12px_35px_rgba(120,72,30,.08)] transition hover:-translate-y-1.5 hover:shadow-xl"><div className="relative aspect-[16/10] overflow-hidden bg-gradient-to-br from-orange-100 to-indigo-100">{item.preview_image ? <img src={item.preview_image} alt={item.title} loading="lazy" className="h-full w-full object-cover transition duration-500 group-hover:scale-105" /> : <Presentation size={58} className="absolute inset-0 m-auto text-indigo-400" />}<DiscountRibbon item={item} className="absolute right-3 top-3 rounded-full bg-rose-500 px-3 py-1 text-[10px] font-black text-white" /></div><div className="p-4"><small className="font-bold text-orange-600">{item.grade.title} · {item.book.title}</small><h3 className="mt-2 line-clamp-2 min-h-12 font-black leading-6 text-slate-900">{item.title}</h3><div className="mt-4 flex items-center justify-between border-t border-slate-100 pt-3"><PowerpointPrice item={item} finalPriceClassName="text-indigo-700" originalPriceClassName="text-[10px] text-slate-400" /><span className="text-xs font-bold text-slate-400">مشاهده و خرید</span></div></div></Link>)}</div> : <Link href="/powerpoints" className="mt-7 flex min-h-52 flex-col items-center justify-center rounded-[2rem] border-2 border-dashed border-orange-200 bg-white/80 p-8 text-center"><span className="flex h-20 w-20 items-center justify-center rounded-3xl bg-gradient-to-br from-orange-100 to-indigo-100 text-indigo-600"><Presentation size={38} /></span><strong className="mt-5 text-lg text-slate-900">ویترین پاورپوینت‌های درسکا</strong><span className="mt-2 text-sm text-slate-500">پاورپوینت‌های منتشرشده همراه تصویر و قیمت در این قسمت نمایش داده می‌شوند.</span></Link>}
        </div>
      </section>
      ...
      <section className="px-4 py-10 sm:py-18">
        <div className="mx-auto max-w-[1480px]">
          ...
          <h2 className="mt-9 text-center text-lg font-black text-[#242238] sm:text-lg">پیشنهاد می‌کنیم از این بخش‌ها دیدن کنید</h2>
          ...
          <div className="mt-7 grid overflow-hidden rounded-3xl border border-slate-100 bg-white shadow-[0_8px_30px_rgba(36,34,56,0.06)] sm:grid-cols-2 lg:grid-cols-4">
...
function SectionTitle({ title }: { title: string }) {
  return <div className="text-center"><h2 className="text-lg font-black text-[#242238] sm:text-xl">{title}</h2><span className="mx-auto mt-3 block h-1 w-10 rounded-full bg-teal-300" /></div>;
}
